...
import {
  ...
  Twitter, Instagram as InstagramIcon, Linkedin, MessageCircle, Mail, Phone,
...
/**
 * InstagramAnalysis.tsx - Fraudara.pro
 * Optimized for SEO (Instagram).
 * 100% Fidelity to Home.tsx and AmazonAnalysis.tsx logic.
...
const InstagramAnalysis: React.FC = () => {
  ...
  const seoDescription = `Is ${brand} a legitimate platform? Read our expert analysis on ${brand}'s safety, phishing risks, and account security in ${currentYear}. Protect your data with Fraudara.`;
  ...
  const handleVerification = async () => {
    ...
    try {
      ...
      let ts = data.trustScore || 85;
      if (data.status === 'safe' && ts < 85) ts = Math.max(85, ts + 5);
      else if (data.status === 'danger' && ts > 40) ts = Math.min(40, ts - 10);
      else if (data.status === 'suspicious') ts = Math.max(40, Math.min(84, ts));
      ...
      setResult({
        ...
        message: 'Instagram is a highly secure global social network.',
        complaints: 0, trustScore: 98, verificationTime: '—',
      });
    } finally {
  ...
  "url": "https://www.instagram.com"
          ...
          "reviewBody": "Instagram is a widely used social media platform owned by Meta, offering a secure environment for sharing content and connecting with others. While the platform itself is legitimate, users should remain cautious of scams, fake profiles, and phishing links that can appear through messages and ads."
        },

        {
          ...
          "mainEntity": [
            {
              "@type": "Question",
              "name": "How do I know if an Instagram link is safe?",
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "Check the domain carefully. If it's anything other than instagram.com (like instagr-am.net), it's a scam. Use Fraudara to be 100% sure."
              }
            },
            {
              "@type": "Question",
              "name": "Is it safe to give my password to third-party apps?",
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "No. Never share your Instagram credentials with apps promising 'followers' or 'who viewed your profile'. These are almost always data-stealing traps."
              }
            },
            {
              "@type": "Question",
              "name": "What is the most common scam on Instagram?",
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "Phishing via DM or email claiming your account will be deleted for 'copyright infringement' is currently the most widespread scam."
              }
            }
          ]
        },
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="py-20 md:py-32 text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl md:text-2xl text-blue-100/80 max-w-2xl mx-auto mb-10 leading-relaxed font-medium px-4">
              Don't lose your account to phishing. Our AI scans {brand}'s security markers in real-time.
            </p>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <div className="flex-1 flex items-center gap-3 px-4">
                  ...
                  <input
                    ...
                    placeholder="Enter Instagram profile or link..."
      ...
      <main id="main-content" className="max-w-4xl mx-auto px-4 py-12">
        ...
        {isVerifying && (
          <div className="bg-white rounded-3xl shadow-xl p-12 text-center animate-pulse border-2 border-blue-100">
            ...
            <p className="text-gray-500">Scanning {searchQuery} for phishing patterns and SSL validity...</p>
          </div>
        )}
          ...
          <div className={`bg-white rounded-3xl shadow-xl border-2 ${cfg?.border} overflow-hidden mb-12`}>
            ...
            <div className="p-8 border-t border-gray-100">
              <h3 className="text-xl font-bold flex items-center gap-2 mb-8">
                <FileText className="w-5 h-5 text-blue-600" /> Instagram Security Report
              </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    ...
                    <div className="space-y-2 text-sm text-slate-600">
                      ...
                      <div className="flex justify-between"><span>SSL:</span> <span className="font-semibold">DigiCert Global</span></div>
                    </div>
                  </div>
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    <div className="flex items-center gap-2 mb-4 font-bold text-slate-800"><Globe className="w-5 h-5 text-blue-500" /> Platform Status</div>
                    <div className="space-y-2 text-sm text-slate-600">
                      <div className="flex justify-between"><span>Age:</span> <span className="font-semibold">16 years</span></div>
                      <div className="flex justify-between"><span>Owner:</span> <span className="font-semibold">Meta Platforms</span></div>
                    </div>
                  </div>
                </div>
              ) : (
        ...
        <article className="prose prose-slate max-w-none mb-20">
          ...
          <p className="text-lg text-slate-600 leading-relaxed mb-8">
            ...
            However, its popularity makes it the #1 target for <strong>phishing campaigns</strong>. In {currentYear}, hackers use AI to create 
            perfectly cloned login pages that look exactly like the real Instagram. If you enter your password on one of these, 
            your account is gone in seconds.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-100">
              ...
              <ul className="space-y-2 text-sm text-emerald-700">
                <li className="flex items-center gap-2"><Check className="w-4 h-4" /> Two-Factor Authentication (2FA)</li>
                <li className="flex items-center gap-2"><Check className="w-4 h-4" /> Advanced Login Alerts</li>
                <li className="flex items-center gap-2"><Check className="w-4 h-4" /> Encrypted Direct Messages</li>
              </ul>
            </div>
            <div className="bg-rose-50 p-6 rounded-2xl border border-rose-100">
              <h4 className="font-bold text-rose-800 flex items-center gap-2 mb-4"><ThumbsDown className="w-5 h-5" /> Security Cons</h4>
              <ul className="space-y-2 text-sm text-rose-700">
                <li className="flex items-center gap-2"><X className="w-4 h-4" /> High prevalence of "Fake Verification" scams</li>
                <li className="flex items-center gap-2"><X className="w-4 h-4" /> Bot accounts spreading malicious links</li>
                <li className="flex items-center gap-2"><X className="w-4 h-4" /> Phishing via "Copyright Infringement" emails</li>
              </ul>
            </div>
          </div>

          <h3 className="text-2xl font-black mb-4">Why Manual Verification Fails</h3>
          <p className="mb-6">
            Scammers now use "homograph attacks"—using characters from different alphabets that look identical to English letters (e.g., using a Cyrillic 'а' instead of a Latin 'a'). 
            To the human eye, it's <code>instagram.com</code>. To the browser, it's a completely different malicious server. 
            <strong> Fraudara.pro</strong>'s AI detects these microscopic anomalies instantly.
          </p>
        </article>
        ...
        <div className="bg-gradient-to-br from-indigo-900 to-blue-900 rounded-3xl p-8 my-16 text-white shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h3 className="text-3xl font-black mb-4">Your Identity is at Stake.</h3>
            <p className="text-blue-100 text-lg mb-8 max-w-2xl leading-relaxed">
              Account theft on Instagram is at an all-time high. Don't let years of memories and followers vanish in one click. 
              <strong> Fraudara.pro</strong> is your digital bodyguard.
            </p>
            <a href="https://fraudara.pro" className="bg-white text-indigo-900 font-bold px-8 py-4 rounded-xl hover:scale-105 transition-all inline-flex items-center justify-center gap-2 shadow-lg group">
              Scan any link now at Fraudara.pro
        ...
        <section className="mb-20">
          <h2 className="text-3xl font-black mb-10 text-center">Instagram Safety FAQ</h2>
          <div className="space-y-4">
            {[
              { q: `How do I know if an Instagram link is safe?`, a: `Check the domain carefully. If it's anything other than instagram.com (like instagr-am.net), it's a scam. Use Fraudara to be 100% sure.` },
              { q: `Is it safe to give my password to third-party apps?`, a: `No. Never share your Instagram credentials with apps promising "followers" or "who viewed your profile". These are almost always data-stealing traps.` },
              { q: `What is the most common scam on Instagram?`, a: `Phishing via DM or email claiming your account will be deleted for "copyright infringement" is currently the most widespread scam.` }
            ].map((faq, i) => (
        ...
        <section className="bg-slate-100 rounded-3xl p-8 mb-20 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {["Amazon", "Shopee", "AliExpress", "Temu", "TikTok"].map(p => (
          ...
          <div className="flex flex-col md:flex-row items-start gap-8">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm leading-relaxed max-w-xl">
                Expert in digital security and anti-phishing systems. Pablo created Fraudara to give everyday users the same level of protection used by global banks.
              </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-gray-500">Protect your identity and assets with Fraudara Pro.</p>
            </div>
      ...
      {showUpgradeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full text-center relative">
            ...
            <p className="text-gray-500 text-sm mb-6">Don't leave your safety to chance. Upgrade for unlimited real-time protection.</p>
